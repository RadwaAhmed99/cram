...
// Purpose: creating new functions using the following operators
// Author:  1)Rahma Ramadan
//2)Radwa Ahmed
//3)Sara Adel
// Date:    10 August 2018
// Version: 1.0
...
matrix operator*  (matrix mat1, matrix mat2) // Multi if col1 == row2
{
    matrix mat ;
    int data[] = {} ;
    createMatrix(mat2.row,mat1.col,data,mat); //new matrix to store data
...
matrix operator+= (matrix& mat1, matrix mat3)//operator as x=x+y
{
    if (mat1.row==mat3.row&&mat1.col==mat3.col){//checking if the two matrices have the same dimensions
        for (int i=0;i<mat3.row;i++){
            for(int j=0; j<mat3.col;j++){
                mat1.data[i][j]=mat1.data[i][j]+mat3.data[i][j];//changing the values of first matrix and put the sum of matrices instead of it
            }
        }
    }
    else
...
matrix operator-= (matrix& mat1, matrix mat3)//operator as x=x-y
{
    if(mat1.row==mat3.row&&mat1.col==mat3.col){//checking if the two matrices have the same dimensions
         for (int i=0;i<mat3.row;i++){
            for(int j=0; j<mat3.col;j++){
                    if(mat3.data[i][j]>mat1.data[i][j]){//checking which matrix has biggest values
                        mat1.data[i][j]=mat3.data[i][j]-mat1.data[i][j];//changing the values of first matrix and put the minus of matrices instead of it
                    }
                    else {
...
void operator++ (matrix& mat)//operator to increase the value of the matrix by 1
{
    for(int i=0;i<mat.row;i++){
        for (int j=0;j<mat.col;j++){
            mat.data[i][j]=mat.data[i][j]+1;//storing new values of matrix in mat
        }
    }
...
void operator-- (matrix& mat1)//operator to decrease the value of the matrix by 1
{
    for(int i=0;i<mat1.row;i++){
        for(int j=0;j<mat1.col;j++){
            mat1.data[i][j]=mat1.data[i][j]-1;//storing new values of matrix in mat1
        }
    }
}
